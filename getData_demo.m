...
for i = 1:length(channelNames)
    ...
    getData(site,cha,days,units);

    % Reformat daily files for easier reading in MATLAB.
    reformatData(site,channelNames{i},days,units);
end
...
for i = 1:length(channelNames)
    days = channelDays.(channelNames{i});
    data = fillGaps(site, channelNames{i}, days, units);

    figure(i);clf;
    set(gcf,'Position',[1 1 900 300]);
    plotGapFilled(data, site, channelNames{i}, days, units);
end